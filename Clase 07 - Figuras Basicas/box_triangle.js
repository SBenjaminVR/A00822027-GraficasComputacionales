...
function createPacman(gl) {
    ...
    let amp = 0.5;
    let finBoca = (Math.PI * 2) - (Math.PI / 4);

    for (let i = Math.PI / 4; i < finBoca; i += 0.1) {
        verts.push(amp * Math.cos(i));
        verts.push(amp * Math.sin(i));
        verts.push(0.0);
    }
    ...
    // The resulting object contains the vertexbuffer, the size of the vertex structure (3 floats, x, y, z), the number of vertices to be drawn, the the primitive to draw.
    let circle = {buffer:vertexBuffer, vertSize:3, nVerts: numVerts, primtype:gl.TRIANGLE_FAN};
    return circle;
}